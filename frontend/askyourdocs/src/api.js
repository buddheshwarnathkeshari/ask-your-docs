// simple api wrapper used by the frontend components
const API_ROOT = "/api";
...
export async function uploadDocument(file, projectId) {
  // IMPORTANT: use FormData and DO NOT set Content-Type. Browser will add boundary.
  ...
  const res = await fetch(`${API_ROOT}/documents/upload/`, {
    method: "POST",
    body: fd,
    // do not set headers -> browser handles multipart
  });
  ...
  if (!res.ok) {
    ...
    try {
      ...
      // backend validation error shape e.g. { file: ["..."] }
...
// Add this to src/api.js (with the other functions)
